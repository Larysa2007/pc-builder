    ...
    if request.method == "POST" and budget_str:
        try:
            ...
            if budget < 5000:
                ...
                # 🔹 ПІДБІР КОМПОНЕНТІВ СУВОРО В МЕЖАХ БЮДЖЕТУ
                ...
                if not cpu:
                    ...
                    if not motherboard:
                        ...
                        gpu = Gpu.objects.filter(
                            price__lte=budget * ratio['gpu']
                        ).order_by('-price').first()

                        storage = Storage.objects.filter(
                            price__lte=budget * ratio['ssd']
                        ).order_by('-price').first()

                        psu = Psu.objects.filter(
                            price__lte=budget * ratio['psu']
                        ).order_by('-price').first()
...
# (залишаємо якщо треба, але ти казала що не використовуєш кнопку)
